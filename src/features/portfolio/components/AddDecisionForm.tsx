...
interface FormBodyProps {
  ...
  updateAlternative: (i: number, field: keyof Alternative, value: string | string[]) => void
  addProCon: (altIndex: number, type: 'pros' | 'cons') => void
  updateProCon: (altIndex: number, type: 'pros' | 'cons', itemIndex: number, value: string) => void
  removeProCon: (altIndex: number, type: 'pros' | 'cons', itemIndex: number) => void
  chosenName: string
  setChosenName: (v: string) => void
  chosenRationale: string
  setChosenRationale: (v: string) => void
  tradeoffs: string[]
  addTradeoff: () => void
  updateTradeoff: (i: number, v: string) => void
  removeTradeoff: (i: number) => void
  evidenceLink: string
  setEvidenceLink: (v: string) => void
  isSaving: boolean
  onCancel: () => void
}
...
function DecisionFormBody({
  ...
  problem, setProblem,
  alternatives, addAlternative, removeAlternative, updateAlternative,
  addProCon, updateProCon, removeProCon,
  chosenName, setChosenName,
  chosenRationale, setChosenRationale,
  tradeoffs, addTradeoff, updateTradeoff, removeTradeoff,
  evidenceLink, setEvidenceLink,
  isSaving, onCancel,
}: FormBodyProps) {
  return (
    <>
...
export function AddDecisionForm({ repositoryUrl, existingDecisions, onSuccess, onSaved, autoOpen = false, inline = false }: AddDecisionFormProps) {
  ...
  // Inline mode: render form directly without modal backdrop
  if (inline) {
    return (
      <form onSubmit={handleSubmit} className="space-y-0">
        ...
        <DecisionFormBody {...formBodyProps} />
      </form>
    )
  }
  ...
  return (
    <>
      {/* Add Decision Button - only show if not auto-opening */}
      {!autoOpen && (
        <button
